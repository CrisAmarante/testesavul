...
class InspecaoVeicular {
  constructor() { 
    ...
    // Expõe o método mostrarModalConferir globalmente para o callback
    window.mostrarModalConferir = this.mostrarModalConferir.bind(this);
  }
  ...
  resetarFormulario() { 
    ...
    if (getEl('terminal')) getEl('terminal').value = '';
  ...
  async enviarInspecao() {
    ...
    const dadosEnvio = { 
      ...
      limpeza: dados.itens.limpeza,    // ALTERADO: USB -> LIMPEZA
  ...
  _executarConsultaInspecao(params) {
    return new Promise((resolve, reject) => {
      ...
      // Salva o this atual para usar dentro do callback
      const self = this;
      ...
      window[callbackName] = (dados) => {
        if (dados && dados.erro) {
          ...
          self.mostrarModalConferir(dados || [], currentUserRole, params);
        }
  ...
  // MÉTODO MOSTRAR MODAL - AGORA DENTRO DA CLASSE
  mostrarModalConferir(inspecoes, role, params) {
    ...
    if (!document.getElementById('filtros-inspecao')) {
      ...
      document.getElementById('btn-aplicar-filtros-inspecao').addEventListener('click', () => {
        ...
        this.conferirInspecoesComFiltro(dataInicio, dataFim, carro, fiscalFiltro);
      });
    ...
    // SE TIVER RESULTADO, MONTA A LISTA
    ...
    inspecoes.forEach(ins => {
      const itensDefeito = [];
      if (ins.thoreb && ins.thoreb.status === 'DEFEITO') itensDefeito.push(`THOREB: ${ins.thoreb.obs || 'sem obs'}`);
      if (ins.elevador && ins.elevador.status === 'DEFEITO') itensDefeito.push(`ELEVADOR: ${ins.elevador.obs || 'sem obs'}`);
      if (ins.limpeza && ins.limpeza.status === 'DEFEITO') itensDefeito.push(`LIMPEZA: ${ins.limpeza.obs || 'sem obs'}`);  // ALTERADO: USB -> LIMPEZA
      if (ins.ventilador && ins.ventilador.status === 'DEFEITO') { 
    ...
    if (html === '<div style="margin-bottom: 12px; text-align: right;"><button id="exportar-lista" class="btn-secundario">📋 Exportar para texto</button></div><div id="lista-inspecoes-conteudo"></div>') {
      container.innerHTML = `<div style="text-align: center; padding: 30px 10px; font-weight: 500;">Nenhum defeito encontrado nas inspeções deste período.</div>`;
    } else {
    ...
    if (exportBtn) {
      ...
      newExportBtn.addEventListener('click', () => { 
        const texto = this.gerarTextoExportacao(inspecoes, role); 
  ...
  gerarTextoExportacao(inspecoes, role) {
...
// Função global para fechar o modal (mantida para compatibilidade)
